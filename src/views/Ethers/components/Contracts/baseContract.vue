<template>
  ...
  <div class="code-comment mrg-bot">
    ...
    deployTx?: null | TransactionResponse)
  </div>
...
<script setup lang="ts">
  ...
  const newBaseContractFun = () => {
    baseContract = new ethers.BaseContract(stakingRewardsAddress, stakingRewardsAbi.output.abi, Store.singer);
  ...
  const dataHelpeFun = async (row) => {
    switch (row.name) {
      ...
      case "deploymentTransaction":
        ...
        // helloworldAbi
        const _contracts = new ethers.Contract(_addr, helloworldAbi.abi, Store.singer);
        ...
        row.result = '见控制面板控制台输出,此方法适用于ethers部署的文件，不适用remix';
        break;
      case "emit":
        // 触发质押事件
      ...
      case "getDeployedCode":
        const _getDeployedCode = await Store.contracts.stakingRewards.getDeployedCode();
      ...
      case "getEvent":
        ...
        break;
      case "getFunction":
        const _getFunction = await Store.contracts.stakingRewards.getFunction("setRewardsDuration");
        console.log('getFunction', _getFunction);
        break;
      case "listenerCount":
        const _listenerCount = await Store.contracts.stakingRewards.listenerCount("SetDuration");
        console.log('listenerCount', _listenerCount);
        row.result = _listenerCount
        break;
      case "listeners":
        const _listeners = await Store.contracts.stakingRewards.listeners("Staked");
      ...
      case "off":
        ...
        break;
      case "on":
        await Store.contracts.stakingRewards.setRewardsDuration(
          1000
        );
      ...
      case "removeAllListeners":
        await Store.contracts.stakingRewards.removeAllListeners("SetDuration", listener);
        row.result = '可不传参数移除所有'
        break;
      case "removeListener":
        await Store.contracts.stakingRewards.removeListener("SetDuration", listener);
        break;
      case "waitForDeployment":
        ...
        break;
      case "buildClass":
        const _buildClass = await ethers.BaseContract.buildClass(stakingRewardsAbi.output.abi);
        console.log('buildClass:', _buildClass);
        row.result = '控制面板控制台输出'
        break;
      case "from":
        const _from = await ethers.BaseContract.from(stakingRewardsAddress, stakingRewardsAbi.output.abi, Store.singer);
