<template>
  <div class="tab-container">
    ...
    <el-card class="mrg-top">
      ...
      <el-button class="mrg-bot" type="primary" @click="encodeBase58Fun"
        >encodeBase58</el-button
      >
      <br />
      <el-button type="primary" @click="decodeBase58Fun"
        >decodeBase58(暂时解码err)</el-button
      >
      <br />
    </el-card>
    <el-divider />
    ...
    <el-card class="mrg-top">
      ...
      <el-button class="mrg-bot" type="primary" @click="base64EncodingFun"
        >base64Encoding</el-button
      >
      <br />
    ...
    <el-card class="mrg-top">
      ...
      <div class="code-comment">
        BytesLike => DataHexString(偶数长度十六进制字符串) |
        Uint8Array(字节数组) :二进制数据的通用表示形式 <br />
    ...
    <el-card class="mrg-top">
      ...
      <div class="code-comment">
        BigNumberish =>string|Numeric 表示大数字的联合类型string,number,bigint
        ("123"、456、789n) <br />
    ...
    <el-card class="mrg-top">
      ...
      <div class="mrg-bot">
        <el-button type="primary" @click="definePropertiesFun"
          >defineProperties</el-button
        >
        <span class="mrg-lef">defineProperties 只读属性</span>
      </div>
      <div>
        <el-button type="primary" @click="resolvePropertiesFun"
          >resolveProperties</el-button
        >
    ...
    <el-card class="mrg-top">
      ...
      <div class="code-comment mrg-bot">
        RlpStructuredData | RLP 编码结构，可以是字符串或 RlpStructuredData 数组
        | "cat" 或 ["cat", ["dog"]] <br />
        RlpStructuredDataish | 扩展类型，支持 Uint8Array | new
        Uint8Array([0x01])
      </div>
      <h4 class="mrg-bot">6.2 函数</h4>
      <div class="code-comment mrg-bot">
        <el-button type="primary" @click="decodeRlpFun"
          >decodeRlp =>("0x8568656c6c6f")</el-button
        >
        <el-button type="primary" @click="encodeRlpFun"
          >encodeRlp=>("hello")</el-button
        >
      </div>
    </el-card>
    <el-divider />
    ...
    <el-card class="mrg-top">
      <h3 class="mrg-bot">7.1 CONSTANTS</h3>
      <div class="code-comment mrg-bot">
        Utf8ErrorFuncs⇒ Record< "error" | "ignore" | "replace" , Utf8ErrorFunc >
        <br />
        ‌数据完整性‌：replace 会保留所有字节（替换非法部分），ignore
        直接丢弃，error 中断处理‌<br />
        ‌典型应用‌：文件解析优先用 replace，调试时用 error，日志处理可选 ignore‌
      </div>
      <h3 class="mrg-bot mrg-top">7.2 TYPES</h3>
      <h4 class="mrg-bot mrg-top">
        UnicodeNormalizationForm =>"NFC" | "NFD" | "NFKC" |
        "NFKD"(Unicode标准化形式对照表)
      </h4>

      <stringsAndUTF8></stringsAndUTF8>
      <h4 class="mrg-bot">FUNCTIONS</h4>
      <utf8Table></utf8Table>
    </el-card>
    <el-divider />
...
import stringsAndUTF8 from "./stringsAndUTF8.vue";
...
const base64EncodingFun = () => {
  ...
  console.log(
    `ethers.encodeBase64(new Uint8Array([0x12, 0x34])): `,
    ethers.encodeBase64(new Uint8Array([0x12, 0x34]))
  );
  ...
  console.log(
    ...
    ethers.encodeBase64(ethers.toUtf8Bytes("Hello World!!"))
  );
  const result = ethers.decodeBase64("SGVsbG8gV29ybGQhIQ==");
  console.log(
    'ethers.decodeBase64("SGVsbG8gV29ybGQhIQ=="): ',
    ethers.toUtf8String(result)
  );
};
