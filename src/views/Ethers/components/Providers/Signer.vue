  ...
  try {
    ...
    switch (row.name) {
      case "authorize":
        ...
        // const _authorize = await Store.signer.authorize(authParams);
        row.result = processAuthResult("不支持");
        break;
      ...
      case "populateAuthorization":
        try {
          ...
          const populatedAuth = await Store.signer.populateAuthorization(
            authRequest
          );
      ...
      case "signTypedData":
        try {
          ...
          const signature = await Store.signer.signTypedData(
            domain,
            types,
            value
          );
