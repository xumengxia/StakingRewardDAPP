<template>
  ...
  <div class="code-comment mrg-bot">
    new BrowserProvider(ethereum: Eip1193Provider, network?: Networkish, options?: BrowserProviderOptions)
  </div>
  <el-button class="mrg-bot" type="primary" @click="newBrowserProviderFun">new BrowserProvider</el-button>
  <el-divider />
  ...
  <el-button class="mrg-bot" type="success" @click="demonstrateContractRunnerMethods">演示 ContractRunner 方法</el-button>
  <el-divider />

  <h4 class="mrg-bot">interface Eip1193Provider 接口</h4>
  <h4 class="mrg-bot">METHODS:</h4>
  <el-button class="mrg-bot" type="primary" @click="requestFun">request</el-button>
  <el-divider />
...
<script setup lang="ts">
        ...
        case "hasSigner":
          try {
            if (Store.currentAccount) {
              const hasSigner = await Store.provider.hasSigner(Store.currentAccount);
  ...
  const demonstrateContractRunnerMethods = async () => {
    try {
      ...
      const stakeGas = await Store.contracts.stakingRewards.setRewardsDuration.estimateGas(
        1000
      );
  ...
  const requestFun = async () => {
    try {
      ...
      try {
        ...
        console.log("⚠️ 交易收据获取失败（正常，因为哈希不存在）:", error.message);
      }
      ...
      try {
        const storage = await window.ethereum.request({
          ...
          params: [
            Store.contracts.stakingRewards?.target || '0x0000000000000000000000000000000000000000',
            '0x0',
            'latest'
          ]
        });
      ...
      try {
        const logs = await window.ethereum.request({
          ...
          params: [{
            fromBlock: '0x0',
            toBlock: 'latest',
            address: Store.contracts.stakingRewards?.target || '0x0000000000000000000000000000000000000000',
            topics: []
          }]
        });
      ...
      alert("✅ Eip1193Provider request 方法演示完成！请查看控制台了解详细信息。");
