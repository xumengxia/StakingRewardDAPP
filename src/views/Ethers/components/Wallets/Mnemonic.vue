<template>
  ...
  <el-button class="mrg-bot" type="primary" @click="fromEntropy"
    >fromEntropy</el-button
  >
  <div class="code-comment mrg-bot">
    .fromPhrase(phrase: string, password?: null | string, wordlist?: null |
    Wordlist)⇒ Mnemonic
  </div>
  <el-button class="mrg-bot" type="primary" @click="fromPhrase"
    >fromPhrase</el-button
  >
  <el-divider />
  <h4>METHODS:</h4>
...
const fromEntropy = () => {
  console.log(ethers.fromEntropy(entropy));
};
const fromPhrase = async () => {
  console.log("fromPhrase", baseWallet);
};
const tableData = [
  {
    name: "authorize",
...
const processAuthResult = (result) => {
